...
# ///
"""
Servidor MCP — auditoria-skills
Expone las 20 SKILLs de auditoría interna como herramientas MCP.
Ejecutar: uv run mcp/server.py
"""
...
async def handle_call_tool(
    ...
    # ── obtener_skill ──────────────────────────────────────────────────────────
        ...
        if not skill_path.exists():
            return [
                types.TextContent(
                    type="text",
                    text=f"Archivo de SKILL no encontrado en el repositorio: {skill['path']}",
                )
            ]
    ...
    # ── buscar_skills ──────────────────────────────────────────────────────────
        ...
        for s in results:
            ...
            fw = " · ".join(s.get("frameworks", []))
            lines.append(f"- **{s['name']}** [{tipo_str} / {cat}]")
            lines.append(f"  Marcos: {fw}")
    ...
    async with stdio_server() as (read_stream, write_stream):
        await server.run(
            ...
            InitializationOptions(
                ...
                server_version=CATALOG.get("version", "1.0.0"),
